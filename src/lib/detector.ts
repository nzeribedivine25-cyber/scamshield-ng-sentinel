// ============================================================
// ScamShield NG — Detection Engine v3.0
// Layer 1: Whitelist → Layer 2: Community Blacklist (Supabase)
// Layer 3: Static Blacklist → Layer 4: Heuristics → Layer 5: Gemini AI
// ============================================================
...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Ind0c2xuYW5tcmlqY21nYWJucGdnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI5MzI1MjcsImV4cCI6MjA5ODUwODUyN30.oGpdYrcvClhromIbDLpuV5YVcaiek5LeZC_wOg_eUuo';
const COMMUNITY_THRESHOLD = 3;

async function getCommunityReport(url: string): Promise<number> {
  try {
    ...
    if (data && data.length > 0) return data[0].report_count;
    return 0;
  } catch {
    return 0;
  }
...
export async function submitCommunityReport(url: string): Promise<boolean> {
  try {
    const existing = await fetch(
      `${SUPABASE_URL}/rest/v1/reported_urls?url=eq.${encodeURIComponent(url)}&select=id,report_count`,
      {
        headers: {
          apikey: SUPABASE_ANON_KEY,
          Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
...
function isStaticBlacklisted(input: string): boolean {
...
export async function detectScam(input: string): Promise<DetectionResult> {
  ...
  if (domain && isWhitelisted(domain)) {
    return { verdict: 'safe', explanation: `${domain} is a verified, trusted Nigerian or global platform.`, detectedBy: 'whitelist' };
  }

  const reportCount = await getCommunityReport(trimmed);
  if (reportCount >= COMMUNITY_THRESHOLD) {
    return { verdict: 'scam', explanation: `This has been reported as a scam by ${reportCount} ScamShield users. Avoid it.`, detectedBy: 'community' };
  }

  if (isStaticBlacklisted(trimmed)) {
    return { verdict: 'scam', explanation: 'This matches a known Nigerian scam pattern. Do not click or share any personal details.', detectedBy: 'blacklist' };
  }

  if (domain && isURLShortener(domain)) {
    return { verdict: 'suspicious', explanation: 'This is a shortened link hiding the real destination. Scammers often use these.', detectedBy: 'heuristic' };
  }

  if (domain && hasTyposquatting(domain)) {
    return { verdict: 'scam', explanation: 'This domain is impersonating a real Nigerian bank or platform. It is likely a phishing site.', detectedBy: 'heuristic' };
  }

  if (hasDangerPhrases(trimmed)) {
    return { verdict: 'scam', explanation: 'This message asks for sensitive info like BVN, PIN, or OTP — a clear Nigerian scam tactic.', detectedBy: 'heuristic' };
  }

  if (hasSuspiciousPhrases(trimmed)) {
    return { verdict: 'suspicious', explanation: 'This uses language commonly found in Nigerian scam attempts. Verify before trusting it.', detectedBy: 'heuristic' };
  }

  try {
    return await analyzeWithGemini(trimmed);
  } catch {
    return { verdict: 'suspicious', explanation: 'We could not fully analyze this right now. Treat with caution until you can verify.', detectedBy: 'ai' };
  }
}
